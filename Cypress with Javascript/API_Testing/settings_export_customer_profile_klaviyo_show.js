...
describe('Settings' , function() {
    ...
    it('Show Export Customer Profile Klaviyo', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/settings/export/customer_profile/klaviyo`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var customerProfileKlaviyo = response.body.export_customer_profile_klaviyo_config
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('export_customer_profile_klaviyo_config').to.satisfy(expectObjectOrNull)
            expect(customerProfileKlaviyo).to.have.property('list_id').that.satisfies(expectStringOrNull)            
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_settings_export_customer_profile_klaviyo_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/settings/export/customer_profile/klaviyo`,
