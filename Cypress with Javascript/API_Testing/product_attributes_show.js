...
describe('Product', function() {
    ...
    it('Show Product Attributes', function() {   
        cy.request ({
            method: 'GET',
            url: `${api_url}s/${shop_unique}/products-attrs`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            }
        }).then(response => {
            const ProductAttributes = response.body.attributes
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('attributes')
            expect(ProductAttributes).to.exist
        })  
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_product_attributes_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/products-attrs`,
