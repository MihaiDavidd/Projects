...
describe('Enums', function() {
    ...
    it('Show Customer Events list', function() {   
        cy.request ({
            method: 'GET',
            url: `${api_url}enums/customer-events`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var customerEvents = response.body.customer_events
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('customer_events').to.satisfy(expectObjectOrNull)
            expect(customerEvents).to.satisfy(expectArrayOrNull)
        })  
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_enums_customer_events_list_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}enums/customer-events`,
