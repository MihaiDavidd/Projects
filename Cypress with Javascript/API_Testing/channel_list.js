...
describe('Channel', function() {
    ...
    it('Show Channels list', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/channels`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            }
        }).then(response => {
            var channel = response.body.channel
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('channel').to.satisfy(expectObjectOrNull)
            const channelTable = channel.table
            for (let i = 0; i < channelTable.length; i++) {
                const table = channelTable[i]
                expect(table).to.have.property('channel').to.satisfy(expectStringOrNull)  
            }
        })
    })
    ...
    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_channel_list_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/channels`,
