...
describe('Settings' , function() {
    ...
    it('Show Email Provider Klaviyo', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/settings/email_provider/klaviyo`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var showKlaviyo = response.body.klaviyo_config
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('klaviyo_config').to.satisfy(expectObjectOrNull)
            expect(showKlaviyo).to.have.property('api_key').that.satisfies(expectStringOrNull)
            expect(showKlaviyo).to.have.property('from_email').that.satisfies(expectStringOrNull)
            expect(showKlaviyo).to.have.property('from_name').that.satisfies(expectStringOrNull)
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_settings_email_provider_klaviyo_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/settings/email_provider/klaviyo`,
