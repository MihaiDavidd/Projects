import {expectNumberOrNull} from '../support/sharedVariables/api_variables'
...
describe('Stats', function() {
    ...
    it('Show Stats', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/stats/`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var showStats = response.body.stats
            expect(response.status).to.eq(200)
            expect(showStats).to.have.property('customers_count').that.satisfies(expectNumberOrNull)
            expect(showStats).to.have.property('order_count').that.satisfies(expectNumberOrNull)
            expect(showStats).to.have.property('products_count').that.satisfies(expectNumberOrNull)
            expect(showStats).to.have.property('total_revenue_last_year').that.satisfies(expectNumberOrNull)
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_stats_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/stats/`,
