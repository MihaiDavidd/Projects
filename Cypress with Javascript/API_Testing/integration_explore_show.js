...
describe('Integration Explore', function() {
    before(function() {
    ...
    it('Show Explore Integration Status', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/integrations/explore`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var showIntegration = response.body.integration_status
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('integration_status').to.satisfy(expectObjectOrNull)
            expect(showIntegration).to.have.property('connection_string').that.satisfies(expectStringOrNull)
            expect(showIntegration).to.have.property('explore_host').that.satisfies(expectStringOrNull)
            expect(showIntegration).to.have.property('status').that.satisfies(expectStringOrNull)
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_integration_explore_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/integrations/explore`,
