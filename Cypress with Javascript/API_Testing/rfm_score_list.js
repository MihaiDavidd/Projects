...
describe('RFM Score', function() {
    ...
    it('Show RFM Score list', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/rfm-scores`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            }
        }).then(response => {
            var scoreRFM = response.body.rfm_scores
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('rfm_scores').to.satisfy(expectObjectOrNull)
            for (const slugScore of scoreRFM) {
                expect(slugScore).to.have.property('slug').that.satisfies(expectStringOrNull)
                expect(slugScore).to.have.property('rfm_group').that.satisfies(expectNumberOrNull)     
            }
        })  
    })
    ...
    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_rfm_score_list_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/rfm-scores`,
