...
describe('Settings' , function() {
    ...
    it('Show Email Provider Sendgrid', function() {   
        cy.request ({
            method:'GET',
            ...
            var showSendgrid = response.body.sendgrid_config
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('sendgrid_config').to.satisfy(expectObjectOrNull)
            expect(showSendgrid).to.have.property('api_key').that.satisfies(expectStringOrNull)
            expect(showSendgrid).to.have.property('sender_id').that.satisfies(expectNumberOrNull)      
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_settings_email_provider_sendgrid_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/settings/email_provider/sendgrid`,
