...
describe('Cost', function() {
    ...
    it('Show Costs list', function() {   
        cy.request ({
            ...
            var Costs = response.body.costs
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('costs').to.satisfy(expectObjectOrNull)
            expect(Costs).to.satisfy(expectObjectOrNull)
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_cost_list_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/costs`,
