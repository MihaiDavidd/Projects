...
describe('RFM Score', function() {
    ...
    it('Show RFM Group', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/rfm-groups`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            }
        }).then(response => {
            var groupRFM = response.body.rfm_groups
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('rfm_groups').to.satisfy(expectObjectOrNull)
            for (const id of groupRFM) {
                expect(id).to.have.property('id').that.satisfies(expectNumberOrNull)
                expect(id).to.have.property('name').that.satisfies(expectStringOrNull)
                expect(id).to.have.property('rank').that.satisfies(expectNumberOrNull)
                expect(id).to.have.property('internal_slug').that.satisfies(expectStringOrNull)     
            }
        })  
    })
    ...
    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_rfm_group_list_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/rfm-groups`,
