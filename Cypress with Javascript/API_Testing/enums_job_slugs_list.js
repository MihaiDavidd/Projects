...
describe('Enums', function() {
    ...
    it('Show Job Slugs list', function() {   
        cy.request ({
            method: 'GET',
            url: `${api_url}enums/job-slugs`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var jobSlugs = response.body.job_slugs
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('job_slugs').to.satisfy(expectObjectOrNull)
            expect(jobSlugs).to.satisfy(expectArrayOrNull)
        })  
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_enums_job_slugs_list_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}enums/job-slugs`,
