import {expectObjectOrNull} from '../support/sharedVariables/api_variables'
...
describe('Settings' , function() {
    ...
    it('Show Sync Settings', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/settings/sync`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var syncSettings = response.body.sync_settings
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('sync_settings').to.satisfy(expectObjectOrNull)
            expect(syncSettings).to.satisfy(expectObjectOrNull)
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_settings_sync_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/settings/sync`,
