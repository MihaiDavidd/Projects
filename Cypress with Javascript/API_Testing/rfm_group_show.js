...
describe('RFM Score', function() {
    ...
    it('Show RFM Group with valid data', function() {   
        cy.request ({
            method:'GET',
            ...
            var groupRFM = response.body.rfm_group
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('rfm_group').to.satisfy(expectObjectOrNull)
            if (groupRFM) {
                expect(groupRFM).to.have.property('id').that.satisfies(expectNumberOrNull)
                expect(groupRFM).to.have.property('name').that.satisfies(expectStringOrNull)
                expect(groupRFM).to.have.property('rank').that.satisfies(expectNumberOrNull)
                expect(groupRFM).to.have.property('internal_slug').that.satisfies(expectStringOrNull)     
            }
        })  
    })

    it('Show RFM Group with invalid data', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/rfm-groups/100`,
    ...
    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_rfm_group_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/rfm-groups/1`,
                    failOnStatusCode: false,
