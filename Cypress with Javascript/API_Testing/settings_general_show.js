...
describe('Settings' , function() {
    ...
    it('Show General Settings', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/settings/general`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var generalSettings = response.body.general_settings
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('general_settings').to.satisfy(expectObjectOrNull)
            expect(generalSettings).to.have.property('stats_days_offset').that.satisfies(expectNumberOrNull)
            expect(generalSettings).to.have.property('timezone_offset').that.satisfies(expectStringOrNull)          
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_settings_general_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/settings/general`,
