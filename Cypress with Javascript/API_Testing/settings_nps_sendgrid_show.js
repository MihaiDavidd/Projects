...
describe('Settings' , function() {
    ...
    it('Show NPS Sendgrid Settings', function() {   
        cy.request ({
            method:'GET',
            url: `${api_url}s/${shop_unique}/settings/nps/sendgrid`,
            headers: {
                'Authorization': 'Bearer ' + accessToken
            },
        }).then(response => {
            var NPSSendgrid = response.body.nps_sendgrid_config
            expect(response.status).to.eq(200)
            expect(response.body).to.have.property('nps_sendgrid_config').to.satisfy(expectObjectOrNull)  
            expect(NPSSendgrid).to.have.property('design_id').that.satisfies(expectStringOrNull)
            expect(NPSSendgrid).to.have.property('nps_global_list_id').that.satisfies(expectStringOrNull)
            expect(NPSSendgrid).to.have.property('template_id').that.satisfies(expectStringOrNull)
            expect(NPSSendgrid).to.have.property('template_version_id').that.satisfies(expectStringOrNull)
            expect(NPSSendgrid).to.have.property('unsubscribe_group_id').that.satisfies(expectNumberOrNull)
        })
    })

    if (Cypress.env('compareData')) {
        it('Check and compare response with the original', function() {
            cy.readFile(`API_Responses/Responses/response_settings_nps_sendgrid_show_${shop_unique}_${platform}.json`).then(originalResponses => {
                cy.request({
                    method: 'GET',
                    url: `${api_url}s/${shop_unique}/settings/nps/sendgrid`,
