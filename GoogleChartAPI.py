...
timing_data = []
with open("C:\\Users\david\Desktop\GITHUB\AutomateReportGenerationUsingGoogleAPIs\Gathering Data\TestTimingData.csv") as csv_file:
...
for row in timing_data[1:]:
    test_name = row[0]
    if not row[1] or not row[2]: #to skip blanks lines of data
        continue
...
<script type="text/javascript">
  ...
  function drawChart () {
    ...
    var chart = new google.visualization.ColumnChart(document.getElementById('chart_div'));
      chart.draw(data); #to initialize and draw chart
  }
</script>
</head>
<body>
<div id="chart_div" style="width:800; height:600"></div> #div that hold the chart
</body>
</html>""")

chart_data_str = ''
for row in column_chart_data[1:]: #1: to not get the header
    chart_data_str += '%s,\n'%row
